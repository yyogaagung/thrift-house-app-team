...
export default function SemuaProduct(){
    ...
    const fetchData = async () => {
        setIsLoading(true)
        const url = `https://thrifthouse.herokuapp.com:443/api/v1/products?storeId=${params.id}&page=${page}&size=8`;
        try {
          const response = await fetch(url);
          const data = await response.json();
          setTotalProduct(data.data.totalItems);
            setProductFromToko((oldMovies) => {
              if (page === 0) {
 ...
 console.log(totalProduct)
    ...
    return(
        <section className="customcontainer mx-auto px-6 mb-16">
                ...
                {productFromToko.length === totalProduct ? " ": 
                  (
